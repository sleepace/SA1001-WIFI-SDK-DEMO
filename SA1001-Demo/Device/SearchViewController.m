...
@property (nonatomic, strong) SLPBleWifiConfig *con;
@end
...
- (void)viewDidLoad {
    ...
    self.con = [[SLPBleWifiConfig alloc] init];
}
...
- (void)scan {
    ...
    BOOL ret = [SLPBLESharedManager scanBluetoothWithTimeoutInterval:8.0 completion:^(SLPBLEScanReturnCodes code, NSInteger handleID, SLPPeripheralInfo *peripheralInfo) {
        if (code == SLPBLEScanReturnCode_Normal) {
            [weakSelf appendPeripheralInfo:peripheralInfo];
        }else if (code == SLPBLEScanReturnCode_Disable) {
            [Utils showDeviceOperationFailed:-10 atViewController:weakSelf];
        }
    }];
...
//        [self connectDevice:[self.deviceList objectAtIndex:indexPath.row]];
        [self goConfigPageWith:[self.deviceList objectAtIndex:indexPath.row]];
    }else{
        [self scan];
    }
}

- (void)goConfigPageWith:(SLPPeripheralInfo *)peripheralInfo
{
    ConfigWifiViewController *vc = [ConfigWifiViewController new];
    vc.peripheralInfo = peripheralInfo;
